...
| **4** | Siết tương phản nền: worst-case patch + ngưỡng bật scrim | Chữ đạt WCAG AA cả trên nền có vệt sáng cục bộ | 🟡 **XONG PHẦN CHÍNH 26/09.** (1) Đo đúng: squint C3 chấm theo **ô vuông XẤU NHẤT** dưới mỗi dòng (trước: trung bình dòng) và tính cả quầng/bóng chữ (lấy lần đo tốt hơn giữa có/không shadow — quầng neon cùng màu không bị phạt oan); **nền giả khắc nghiệt** `generate_harsh_backdrop_data_uri` (vệt đèn mềm + khối sáng cạnh sắc trên nền tối, mảng tối trên nền sáng, tất định theo mã case), `probe --harsh`. Mốc: **C3 nền khắc nghiệt 159/379**. (2) **Quầng thích ứng** (autofit JS Bước 5, `tendooHalo`): đọc điểm ảnh THẬT của ảnh nền dưới từng dòng chữ đã chốt vị trí, trộn nền bán trong suốt của khối cha, dòng dưới 1.15×ngưỡng WCAG được thêm quầng mềm ngược màu, đậm dần theo mức thiếu — KHÔNG thêm hộp nền (giữ yêu cầu chữ đè thẳng lên ảnh). C3 nền khắc nghiệt 159 → **317/379**, nền thường 351 → **371/379**. (3) Sửa màu gán cứng: tên món menu `#fff` → màu thích ứng vùng; bóng đen gán cứng của hero/subhead ở 12 template → chỉ bật khi vùng thật sự tối (23 chỗ; biến `card_is_dark` mới). CI: mốc thứ ba `tests/squint_baseline_harsh.json` (1/4 suite). **Còn lại (§8):** ~62 case nền khắc nghiệt vẫn dưới ngưỡng theo phép đo (nhiều case mắt vẫn đọc được: chữ tối + quầng sáng trên mảng tối); chữ tô gradient (3d_gold/chrome) không được chấm và không nhận quầng |
| **3R** | Đối chiếu LLM THẬT (GĐ 3 đã làm với "LLM lý tưởng") | Tỉ lệ `hero_parts` qua Cổng 1 ≥ 90%; squint trên plan LLM thật ≥ bộ oracle; không field lạ qua Cổng 2 | 🟡 **Chạy được 26/09 bằng OpenAI API** (người duyệt cấp khoá trong `.env`; `scripts/run_real_llm.py`, 16 brief thật `tests/real_briefs.json`, `gpt-5.4-mini`; ảnh nền thật `gpt-image-2` + gợi ý bố cục mô phỏng corridor từ `geometry`). **Cổng 1: hero_parts 12/16, qua nguyên văn 12/12 (100%)**; Cổng 2 sạch 14/16 (2 dùng sai linh kiện đều rơi về an toàn); chọn template hợp lý (review có câu trích dẫn → quote_spotlight; thuần chữ → maskless). **Squint trên ảnh thật: 6/16 → 8/16 đạt cả 4** sau 4 sửa đổi rút từ LLM thật: prompt (intent phải thuộc template, NÊN dùng linh kiện, tách hero_parts cho tên sản phẩm dài), planner tương thích GPT-5 (`max_completion_tokens`, `reasoning_effort`) + thử lại 1 lần lỗi tạm thời, nới intent (grand_opening nhận big_number_deal/hook_headline; sandwich nhận festive_event), Cổng 3 phủ quyết template làm MẤT field, autofit Bước 5 đảo trắng↔navy khi Python lấy mẫu sai dải (nền khắc nghiệt C3 81 → 91/99). **Còn:** thuần chữ đặt chữ nhỏ trên đỉnh, 3/4 khung trống (b09, b15) → R6 showcase; LLM chọn stat "SALE" thay vì "50%"; nền tông trung bình (bg 0.12–0.28) cả trắng lẫn navy < 4.5 → cần scrim (trái yêu cầu "chữ đè thẳng") — giới hạn đã biết. Chưa chạy Qwen/vLLM của máy chủ |
| **5** | Maskless Mode + **đo** tốc độ thật | Poster thuần chữ đạt §4.5; có số wall-clock thật | 🟡 **Phần local XONG 26/09 — còn đo tốc độ trên máy chủ.** `velocity_blending.denoise_scene_only` (batch 1, bỏ corridor) — trùng khớp đường blend với mask≡0, kiểm bằng mô hình giả trên CPU (`test_maskless.py`, không tải DiT); demo_server rẽ nhánh theo `plan.maskless` (bỏ mã hoá prompt corridor, mask rỗng). `plan.maskless` + `catalog.MASKLESS_INTENTS` (big_number_deal/hook_headline/festive_event), Cổng 2 chặn intent khác; prompt mục 9 buộc nền ÍT CHI TIẾT. Nền giả maskless `generate_lowdetail_backdrop_data_uri` (bokeh + bụi phủ CẢ khung), `probe --maskless`: **C3 118/119** (tiêu đề phẳng), 30/36 (hero_parts) → ràng buộc 2 của §5.3 (Luật 3 hoàn thiện trước) đạt nhờ GĐ 4. Kèm sửa: màu nhấn stat trộn sắc độ tới đủ tương phản (quầng không cứu được ruột chữ to cùng tông nền); màu quầng chọn theo tương phản với CHÍNH màu chữ; probe ẩn chữ bằng inline style (inline !important của Bước 5 từng lọt vào ảnh "nền"); khối màu nhấn chứa chữ nhỏ (ribbon, viên nang) chỉnh sắc độ tới 4.5:1. Mốc CI thứ tư `squint_baseline_maskless.json`. **Còn:** `scripts/bench_maskless.py` (tự từ chối chạy khi không có CUDA) — chạy trên máy chủ để có số wall-clock thật |
| **6** | Template mới — **chỉ cho các ô trống mà bản đồ lần 2 chỉ ra** | Mỗi cái: 2 file + 16-case suite đạt §4.5 | 🟡 **6A+6B XONG 26/09.** Bản đồ phủ sóng lần 3 (intent × khung, sức chứa thẩm mỹ tốt nhất): 4/6 intent phủ 511–699 ký tự ⇒ KHÔNG xây thêm. **6A** `festive_event` chỉ 1 template, 1:1/16:9 gãy ở 64 ký tự → nguyên nhân hình học (dải đầu co còn 18%/28% khi không freetext, badge chiếm chỗ hero) → nới dải đầu: **64 → 699 / 370**; lỗ hổng lấp bằng sửa hình học, **không xây template lễ hội thứ hai** (§3.5). **6B** `quote_spotlight` (testimonial_trust, chuyên biệt): theo thực hành thiết kế (tra cứu 26/09) câu trích dẫn là chữ to nhất (Cấp 1 `.quote-hero`), sao/nhãn/tên người/CTA là Cấp 3, **không subhead** — squint **15/16 đạt cả 4** (customer_feedback_card: 1/33); sức chứa thẩm mỹ 377/235/93/235. customer_feedback_card giữ lại cho đánh giá cần mô tả/ưu đãi đi kèm; prompt chọn giữa 2. **Còn:** (c) sandwich_top/bottom 16:9 sức chứa thẩm mỹ 64/95 → R1 |
...
### 7.1. Subplan phần còn lại (26/09) — cô lập LLM/FLUX, làm được trên máy local trước
...
| Quầng thích ứng chỉ ước lượng nền bằng ảnh gốc + màu khối cha (bỏ qua gradient/backdrop-filter) — lệch vài % so với ảnh chụp; chữ gradient không được xử lý | Trung bình | Biên 1.15× đã bù phần lớn; nếu cần chính xác tuyệt đối: 2 lượt render (chụp nền → quyết định quầng) ở PosterRenderer |
| `customer_feedback_card` C1 1/33 | Trung bình | **Chốt 26/09 (tra cứu thực hành thiết kế):** thẻ đánh giá lấy **câu trích dẫn làm tâm điểm, chữ to nhất**; tên người nhỏ hơn, thương hiệu nhỏ nhất. Đã thử: nâng dải đầu 14→18% (+0.16, 0 case); co subhead theo hero đo thật (sàn 14px): C1 1→9 nhưng C2 33→11 (subhead 14px sát Cấp 3 13px) → không áp. ⇒ **Không sửa được bằng cỡ chữ: cần thiết kế lại bố cục** (quote = điểm neo lớn, "ĐÁNH GIÁ KHÁCH HÀNG" thành nhãn nhỏ) — ứng viên GĐ 6 |
...
- **Thẩm mỹ được bảo đảm bằng 5 luật đo được:** thứ bậc cỡ chữ (Cấp 3 ≤ 0.8×subhead, sàn 13px;
  trần subhead = trần hero ÷ ngưỡng intent khi hero có stat) · tương phản trong dòng qua markup
  `hero_parts` · WCAG trên nền đo thật theo ô xấu nhất + quầng thích ứng · danh mục hiệu ứng/linh
  kiện đóng gắn intent · **squint test 4 điều kiện, 3 mốc CI** (suite, hero_parts, nền khắc nghiệt).
  Hiện trạng 26/09: đạt cả 4 = 82/379 (từ 51); sức chứa thẩm mỹ 150–699 ký tự khi có hero_parts.
