...
| **3R** | Đối chiếu LLM THẬT trên máy chủ (GĐ 3 đã làm với "LLM lý tưởng") | Tỉ lệ `hero_parts` qua Cổng 1 ≥ 90%; squint trên plan LLM thật ≥ bộ oracle; không field lạ qua Cổng 2 | Chờ máy chủ (người duyệt đang ở nhà). Chạy `generate_creative_plan` trên suite brief thật, đo bằng `probe_type_hierarchy` |
...
| R2 | GĐ 7a: ngắt dòng theo nghĩa tiếng Việt (không tách từ ghép / số+đơn vị qua 2 dòng) | probe đếm dòng mồ côi + cặp âm tiết bị tách | local |
