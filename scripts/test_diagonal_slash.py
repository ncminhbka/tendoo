#!/usr/bin/env python3
"""
scripts/test_diagonal_slash.py

Script kiểm thử toàn diện cho template: diagonal_slash (Vát Chéo Năng Động)
- Kiểm tra 16 test cases qua 4 tỉ lệ (1:1, 9:16, 16:9, 4:5) x 4 mức độ dài text.
- Kiểm tra 2 hướng bản gương: orientation='left' và orientation='right'.
- Đo lường kích thước font, chống mất chữ, chống tràn bounding box, chống tràn canvas.
- Tạo báo cáo visual gallery tại: output_tendoo_v3/diagonal_slash_gallery/gallery.html
"""
...
def generate_mock_background(width: int, height: int, theme_hex: str = "#FF3366") -> str:
    """Tạo ảnh nền giả lập phong cách thể thao/công nghệ tối màu để test visual."""
    img = Image.new("RGB", (width, height), color=(8, 12, 22))
    draw = ImageDraw.Draw(img)

    # Thêm dải gradient nhẹ và ánh sáng thể thao
    r = int(theme_hex[1:3], 16)
    g = int(theme_hex[3:5], 16)
    b = int(theme_hex[5:7], 16)

    # Vùng ánh sáng chủ thể bên phía đối diện text
    center_x = int(width * 0.70)
    center_y = int(height * 0.55)
    radius = int(min(width, height) * 0.45)

    for i in range(radius, 0, -25):
...
def inspect_page_metrics(page, width: int, height: int) -> List[Dict[str, Any]]:
    ...
    () => {
      ...
      const els = document.querySelectorAll(
        '.badge-pill, .kicker-tag, .hero-title, .subhead-title, .extra-pill, .freetext-block, .cta-btn, .store-item, .store-info-col, .qr-card-pod'
      );
      for (const el of els) {
...
def build_gallery_html(title_name: str, results: List[Dict[str, Any]]) -> str:
    ...
    for res in results:
        ...
        orient_label = "Gương Phải" if orient == "right" else "Vát Trái"
        ...
        <div class="case-card">
          <div class="card-header">
            ...
            <div class="card-meta">
              <span class="orient-pill">{orient_label}</span>
...
<body>
  <div class="header">
    ...
    <p>Kiểm tra tự động 16 ca thử nghiệm across 4 tỉ lệ & 2 hướng bản gương (Vát Trái vs Gương Phải)</p>
  </div>
  <div class="grid">
    {cards_joined}
  </div>
...
def run_diagonal_slash_test_suite():
    suite_path = Path(__file__).resolve().parent.parent / "tests" / "test_diagonal_slash_suite.json"
    with open(suite_path, "r", encoding="utf-8") as f:
        cases = json.load(f)

    gallery_dir = Path(__file__).resolve().parent.parent / "output_tendoo_v3" / "diagonal_slash_gallery"
    gallery_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print(f"TENDOO V3 - CHẠY KIỂM THỬ TEMPLATE: DIAGONAL_SLASH (16 CASES)")
    print("=" * 80)
    ...
    with sync_playwright() as p:
        ...
        for idx, case in enumerate(cases, 1):
            ...
            style_obj = StyleConfig(
                theme_color=style_dict.get("theme_color", "#FF3366"),
                text_effect=style_dict.get("text_effect", "neon_glow"),
    ...
    gallery_html = build_gallery_html("Vát Chéo Năng Động (Diagonal Slash)", results)
    gallery_file = gallery_dir / "gallery.html"
    with open(gallery_file, "w", encoding="utf-8") as f:
        f.write(gallery_html)

    print("\n" + "=" * 80)
    print("TỔNG KẾT KIỂM THỬ DIAGONAL SLASH:")
    print(f"Gallery Report: file:///{gallery_file.resolve().as_posix()}")
    print("=" * 80)


if __name__ == "__main__":
    run_diagonal_slash_test_suite()
