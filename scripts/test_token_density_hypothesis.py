"""
================================================================================
TENDOO AI - TOKEN DENSITY & MASS DOMINANCE HYPOTHESIS BENCHMARK
Model: FLUX.2 klein 4B Base + Qwen3-4B-FP8 + AE
Platform: 2x NVIDIA A30 (48GB VRAM) / Single GPU Compatible

Objective:
- Test the Token Mass Dominance Hypothesis:
  "Does equalizing / boosting the token count of subtitle & promo slots (>= 480 tokens)
   enable them to survive softmax competition against the product reference and render 100%?"
================================================================================
"""
...
def run_token_density_experiment():
    ...
    output_dir = Path("token_density_results")
    ...
    # Define the 3 Comparative Token Mass Tiers
    test_cases = [
        (
            "Pass A: Baseline Low-Mass (480 / 360 / 320 tokens)",
            [
                {"text": "ÂM THANH ĐỈNH CAO", "t": 10.0, "font": "anton", "w": 640, "h": 192},  # 40x12 = 480 tokens
                {"text": "CHỐNG ỒN CHỦ ĐỘNG", "t": 20.0, "font": "bevietnam", "w": 576, "h": 160},  # 36x10 = 360 tokens
                {"text": "MUA 1 TẶNG 1", "t": 30.0, "font": "pacifico", "w": 512, "h": 160},  # 32x10 = 320 tokens
            ],
        ),
        (
            "Pass B: Equalized Mass (480 / 480 / 480 tokens)",
            [
                {"text": "ÂM THANH ĐỈNH CAO", "t": 10.0, "font": "anton", "w": 640, "h": 192},  # 40x12 = 480 tokens
                {"text": "CHỐNG ỒN CHỦ ĐỘNG", "t": 20.0, "font": "bevietnam", "w": 640, "h": 192},  # 40x12 = 480 tokens
                {"text": "MUA 1 TẶNG 1", "t": 30.0, "font": "pacifico", "w": 640, "h": 192},  # 40x12 = 480 tokens
            ],
        ),
        (
            "Pass C: High-Mass Density (672 / 672 / 672 tokens)",
            [
                {"text": "ÂM THANH ĐỈNH CAO", "t": 10.0, "font": "anton", "w": 768, "h": 224},  # 48x14 = 672 tokens
                {"text": "CHỐNG ỒN CHỦ ĐỘNG", "t": 20.0, "font": "bevietnam", "w": 768, "h": 224},  # 48x14 = 672 tokens
                {"text": "MUA 1 TẶNG 1", "t": 30.0, "font": "pacifico", "w": 768, "h": 224},  # 48x14 = 672 tokens
            ],
        ),
    ]
    ...
    for name, text_configs in test_cases:
        print(f"\n" + "-" * 70)
        ...
        # Product Reference at t=40.0
        ref_tokens_list.append(prod_tokens)
        ref_ids_list.append(prod_ids)
        ...
        for tc in text_configs:
            ...
            print(f"    🔤 [Slot t={tc['t']}] '{tc['text']}' -> Box: {tc['w']}x{tc['h']} ({tok.shape[1]} tokens)")
        ...
        with torch.no_grad():
            ...
            titles.append(name.split(":")[0] + f" ({text_configs[1]['w']}x{text_configs[1]['h']})")

    stitch_horizontal_comparison(
        images=results,
        titles=titles,
        output_path=str(output_dir / "TOKEN_DENSITY_HYPOTHESIS_COMPARISON.png"),
    )


if __name__ == "__main__":
    run_token_density_experiment()
