#!/usr/bin/env python3
"""
scripts/test_customer_feedback_card.py

Script kiểm thử toàn diện cho template: customer_feedback_card (Thẻ Đánh Giá Khách Hàng)
- Kiểm tra 16 test cases qua 4 tỉ lệ (1:1, 9:16, 16:9, 4:5) x 2 hướng (left, right) x nhiều mật độ text.
- Đo lường kích thước font, chống mất chữ, chống tràn bounding box, chống tràn canvas.
- Tạo báo cáo visual gallery tại: output_tendoo_v3/customer_feedback_card_gallery/gallery.html
"""
...
def generate_mock_background(width: int, height: int, orientation: str, theme_hex: str = "#D4AF37") -> str:
    """Tạo ảnh nền studio/lifestyle nghệ thuật với chủ thể ở vị trí đối xứng của card."""
    ...
    # Đặt vùng sáng chủ thể ở đối diện của Feedback Card
    if orientation in ("left", "bottom_left"):
        center_x = int(width * 0.72)
        center_y = int(height * 0.38)
    else:  # right, bottom_right
...
def inspect_page_metrics(page, width: int, height: int) -> List[Dict[str, Any]]:
    ...
    () => {
      ...
      const els = document.querySelectorAll(
        '.top-header-bar, .hero-top-title, .subhead-title, .brand-kicker, .feedback-card, .testimonial-quote, .reviewer-info, .extra-pill, .freetext-block, .cta-btn, .store-item, .qr-col'
      );
      for (const el of els) {
...
def build_gallery_html(title_name: str, results: List[Dict[str, Any]]) -> str:
    ...
    for res in results:
        ...
        orient_labels = {
            "left": "Thẻ Bên Trái",
            "right": "Thẻ Bên Phải (Mirror)",
            "bottom_left": "Thẻ Dưới Trái",
            "bottom_right": "Thẻ Dưới Phải"
        }
...
<body>
  <div class="header">
    ...
    <p>Tổng cộng {len(results)} kịch bản kiểm thử (4 tỉ lệ x 2 hướng Left/Right x Đa mật độ văn bản)</p>
  </div>
  <div class="grid">
    {cards_joined}
  </div>
...
def main():
    ...
    parser = argparse.ArgumentParser(description="customer_feedback_card stress-test runner")
    parser.add_argument("--suite", type=str, default=str(root_dir / "tests" / "test_customer_feedback_card_suite.json"))
    parser.add_argument("--output", type=str, default=str(root_dir / "output_tendoo_v3" / "customer_feedback_card_gallery"))
    ...
    print(f"================================================================================")
    print(f"🚀 BẮT ĐẦU KIỂM THỬ: customer_feedback_card (Tổng số: {len(cases)} test cases)")
    print(f"================================================================================")
    ...
    with sync_playwright() as p:
        ...
        for idx, case in enumerate(cases, 1):
            ...
            plan = TendooCreativePlan(
                ...
                testimonial=case.get("testimonial"),
            ...
            # Re-take exact screenshot if needed or verify
    ...
    # Ghi gallery
    gallery_html = build_gallery_html("customer_feedback_card (Thẻ Đánh Giá Khách Hàng)", results)
    ...
    print(f"\n================================================================================")
    print(f"🏁 HOÀN TẤT KIỂM THỬ customer_feedback_card:")
