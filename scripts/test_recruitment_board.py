#!/usr/bin/env python3
"""
scripts/test_recruitment_board.py

Script kiểm thử toàn diện cho template: recruitment_board (Bảng Tin Tuyển Dụng)
...
- Tạo báo cáo visual gallery tại: output_tendoo_v3/recruitment_board_gallery/gallery.html
"""
...
def generate_mock_background(width: int, height: int, orientation: str = "left", theme_hex: str = "#D4AF37") -> str:
    """Tạo ảnh nền văn phòng/studio tuyển dụng hiện đại với ánh sáng tập trung ở vùng giữa."""
...
def inspect_page_metrics(page, width: int, height: int) -> List[Dict[str, Any]]:
    ...
    () => {
      ...
      const els = document.querySelectorAll(
        '.recruitment-header, .badge-capsule, .hero-title, .subhead-title, .board-card, .col-title, .extra-item, .cta-btn, .store-item, .qr-col'
      );
      for (const el of els) {
...
<body>
  <div class="header">
    ...
    <p>Tổng cộng {len(results)} kịch bản kiểm thử (4 tỉ lệ x 2 hướng Mirror x Đa dạng ngành nghề)</p>
  </div>
  <div class="grid">
    {cards_joined}
  </div>
...
def main():
    ...
    parser = argparse.ArgumentParser(description="recruitment_board stress-test runner")
    parser.add_argument("--suite", type=str, default=str(root_dir / "tests" / "test_recruitment_board_suite.json"))
    parser.add_argument("--output", type=str, default=str(root_dir / "output_tendoo_v3" / "recruitment_board_gallery"))
    ...
    print(f"================================================================================")
    print(f"🚀 BẮT ĐẦU KIỂM THỬ: recruitment_board (Tổng số: {len(cases)} test cases)")
    print(f"================================================================================")
    ...
    with sync_playwright() as p:
        ...
        for idx, case in enumerate(cases, 1):
            ...
            plan = TendooCreativePlan(
                ...
                qr_label=case.get("qr_label", "NỘP CV"),
    ...
    # Ghi gallery
    gallery_html = build_gallery_html("recruitment_board (Bảng Tin Tuyển Dụng)", results)
    ...
    print(f"\n================================================================================")
    print(f"🏁 HOÀN TẤT KIỂM THỬ recruitment_board:")
