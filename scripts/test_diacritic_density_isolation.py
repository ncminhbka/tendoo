"""
================================================================================
TENDOO AI - STACKED DIACRITIC DENSITY & VERTICAL HEIGHT ISOLATION BENCHMARK
Model: FLUX.2 klein 4B Base + Qwen3-4B-FP8 + AE
Platform: 2x NVIDIA A30 (48GB VRAM) / Single GPU Compatible

Objective:
- Isolate whether the failure of 'CHỐNG ỒN CHỦ ĐỘNG' is caused by:
  1. Stacked 2-Layer Diacritics (Ố - Ồ - Ủ - Ộ) vs Simple/No Diacritics
  2. Insufficient Vertical Height Budget for stacked accents
  3. Shortened Word Count with High Per-Character Pixel Allocation

Comparative Passes:
- Pass A (Test B - Control: Simple Diacritics, 17 chars):
    t10="ÂM THANH ĐỈNH CAO" | t20="PIN TRÂU DÙNG LÂU" | t30="MUA 1 TẶNG 1"
- Pass B (Test C - Vertical Height Boost for "CHỐNG ỒN CHỦ ĐỘNG"):
    t10="ÂM THANH ĐỈNH CAO" | t20="CHỐNG ỒN CHỦ ĐỘNG" (Box 640x320 - Tall Vertical Headroom) | t30="MUA 1 TẶNG 1"
- Pass C (Test A - Short Isolated Stacked Diacritics "CHỐNG ỒN"):
    t10="ÂM THANH ĐỈNH CAO" | t20="CHỐNG ỒN" (High per-char resolution) | t30="MUA 1 TẶNG 1"
================================================================================
"""
...
def run_diacritic_isolation_benchmark():
    ...
    output_dir = Path("diacritic_isolation_results")
    ...
    test_passes = [
        (
            "Pass A: Control Simple Diacritics [t20='PIN TRÂU DÙNG LÂU']",
            [
                {"text": "ÂM THANH ĐỈNH CAO", "t": 10.0, "font": "anton", "w": 768, "h": 224},
                {"text": "PIN TRÂU DÙNG LÂU", "t": 20.0, "font": "bevietnam", "w": 768, "h": 224},  # 17 chars, single accents
                {"text": "MUA 1 TẶNG 1", "t": 30.0, "font": "pacifico", "w": 768, "h": 224},
            ],
        ),
        (
            "Pass B: Height Boost for Stacked [t20='CHỐNG ỒN CHỦ ĐỘNG' Tall 640x320]",
            [
                {"text": "ÂM THANH ĐỈNH CAO", "t": 10.0, "font": "anton", "w": 768, "h": 224},
                {"text": "CHỐNG ỒN CHỦ ĐỘNG", "t": 20.0, "font": "bevietnam", "w": 640, "h": 320},  # Tall vertical budget
                {"text": "MUA 1 TẶNG 1", "t": 30.0, "font": "pacifico", "w": 768, "h": 224},
            ],
        ),
        (
            "Pass C: Short Isolated Stacked [t20='CHỐNG ỒN' 2 words]",
            [
                {"text": "ÂM THANH ĐỈNH CAO", "t": 10.0, "font": "anton", "w": 768, "h": 224},
                {"text": "CHỐNG ỒN", "t": 20.0, "font": "bevietnam", "w": 640, "h": 224},  # 9 chars, huge font size
                {"text": "MUA 1 TẶNG 1", "t": 30.0, "font": "pacifico", "w": 768, "h": 224},
            ],
        ),
    ]
    ...
    stitch_horizontal_comparison(
        images=results,
        titles=titles,
        output_path=str(output_dir / "DIACRITIC_DENSITY_ISOLATION_COMPARISON.png"),
    )


if __name__ == "__main__":
    run_diacritic_isolation_benchmark()
