#!/usr/bin/env python3
"""
scripts/test_lifestyle_corner_pod.py

Script kiểm thử toàn diện cho template: lifestyle_corner_pod (Hộp Bo Góc Capsule 4 Góc)
- Kiểm tra 16 test cases qua 4 tỉ lệ (1:1, 9:16, 16:9, 4:5) x 4 hướng góc (bottom_left, bottom_right, top_left, top_right).
- Đo lường kích thước font, chống mất chữ, chống tràn bounding box, chống tràn canvas.
- Tạo báo cáo visual gallery tại: output_tendoo_v3/lifestyle_corner_pod_gallery/gallery.html
"""
...
def generate_mock_background(width: int, height: int, orientation: str, theme_hex: str = "#D4AF37") -> str:
    """Tạo ảnh nền lifestyle nghệ thuật với ánh sáng đối diện vị trí của corner pod."""
    ...
    # Đặt vùng sáng chủ thể ở góc đối diện của Corner Pod
    if orientation in ("bottom_left", "left"):
        center_x = int(width * 0.70)
        center_y = int(height * 0.35)
    elif orientation in ("bottom_right", "right"):
...
def inspect_page_metrics(page, width: int, height: int) -> List[Dict[str, Any]]:
    ...
    () => {
      ...
      const els = document.querySelectorAll(
        '.corner-pod, .badge-pill, .hero-title, .subhead-title, .extra-pill, .freetext-block, .cta-btn, .store-item, .store-info-col, .qr-col'
      );
      for (const el of els) {
...
def build_gallery_html(title_name: str, results: List[Dict[str, Any]]) -> str:
    ...
    for res in results:
        ...
        orient = res.get("orientation", "bottom_left")
...
<body>
  <div class="header">
    ...
    <p>Kiểm tra tự động 16 ca thử nghiệm across 4 tỉ lệ & 4 hướng góc (Dưới Trái, Dưới Phải, Trên Trái, Trên Phải)</p>
  </div>
  <div class="grid">
    {cards_joined}
  </div>
...
def run_lifestyle_corner_pod_test_suite():
    import argparse

    root_dir = Path(__file__).resolve().parent.parent
    parser = argparse.ArgumentParser(description="lifestyle_corner_pod stress-test runner")
    parser.add_argument("--suite", type=str, default=str(root_dir / "tests" / "test_lifestyle_corner_pod_suite.json"))
    parser.add_argument("--output", type=str, default=str(root_dir / "output_tendoo_v3" / "lifestyle_corner_pod_gallery"))
    ...
    print("=" * 80)
    print(f"TENDOO V3 - CHẠY KIỂM THỬ TEMPLATE: LIFESTYLE_CORNER_POD (16 CASES)")
    print("=" * 80)
    ...
    with sync_playwright() as p:
        ...
        for idx, case in enumerate(cases, 1):
            ...
            orient = case.get("orientation", "bottom_left")
            ...
            print(f"[{idx:02d}/{len(cases)}] Chạy: {cid} | ({w}x{h}) {orient.upper()} - {title}")
            ...
            style_obj = StyleConfig(
                ...
                text_effect=style_dict.get("text_effect", "warm_glow"),
                background_tone=style_dict.get("background_tone", "warm_rustic"),
            )
            ...
            bg_data_uri = generate_mock_background(w, h, orientation=orient, theme_hex=style_obj.theme_color)
    ...
    gallery_html = build_gallery_html("Hộp Bo Góc Capsule (Lifestyle Corner Pod)", results)
    gallery_file = gallery_dir / "gallery.html"
    with open(gallery_file, "w", encoding="utf-8") as f:
        f.write(gallery_html)

    print("\n" + "=" * 80)
    print("TỔNG KẾT KIỂM THỬ LIFESTYLE CORNER POD:")
    print(f"Gallery Report: file:///{gallery_file.resolve().as_posix()}")
    print("=" * 80)


if __name__ == "__main__":
    run_lifestyle_corner_pod_test_suite()
