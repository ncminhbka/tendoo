"""
================================================================================
TENDOO AI - PROMPT SPATIAL & STYLISTIC CONTROL BENCHMARK
Model: FLUX.2 klein 4B Base + Qwen3-4B-FP8 + AE
Platform: 2x NVIDIA A30 (48GB VRAM) / Single GPU Compatible

Scientific Objective:
Examine whether the Text Prompt can steer:
  1. Spherical / Curved Text Wrap (Chữ uốn cong ôm quanh quả địa cầu / quả bóng)
  2. Text Size Modulation (Dòng chữ siêu nhỏ tinh tế - Tiny/Subtle scale)
  3. Precise Color Tuning (Đèn neon xanh ngọc lục bảo - Emerald green hologram)
  4. Organic Wave Curvature (Chữ uốn cong theo dải lụa / ruy băng)

Setup:
  - Resolution: 9:16 (576x1024)
  - Guidance: 4.5 | Steps: 50 | Seed: 42 (Shared Initial Noise Latent)
  - Glyph at t = 10.0 (True Tight Crop)
================================================================================
"""
...
# 4 Experimental Spatial Control Scenarios
SCENARIOS = [
    {
        "id": "earth_curved",
        "title": "TEST 1: CHỮ CONG VÒNG QUANH TRÁI ĐẤT",
        "subtitle": "Spherical Curvature / Orbital Text Ring",
        "text": "VIỆT NAM TOÀN CẦU",
        "font": "playfair",
        "prompt": (
            "Không gian vũ trụ sâu thẳm huyền ảo với muôn vàn vì sao lấp lánh, một quả địa cầu Trái Đất 3D phát sáng rực rỡ ở trung tâm, "
            "dòng chữ tiêu đề 3D mạ vàng ánh kim uốn cong tròn ôm vòng quanh đường xích đạo của quả địa cầu như một dải quỹ đạo phát sáng, "
    ...
    {
        "id": "ball_curved",
        "title": "TEST 2: CHỮ CONG ÔM QUẢ BÓNG ĐÁ",
        "subtitle": "Ball Surface Wrap / Neon Fire Glow",
        "text": "CHIẾN THẮNG",
        "font": "anton",
        "prompt": (
            "Sân cỏ bóng đá rực sáng dưới dàn đèn pha sân vận động, một quả bóng đá hiện đại ở tiền cảnh, "
            "dòng chữ tiêu đề 3D đèn neon phát quang màu đỏ rực lửa uốn lượn cong tròn ôm sát theo đường cong của quả bóng, "
            "hiệu ứng quầng sáng rực lửa và khói bụi tương phản cao, phong cách điện ảnh 3D, chi tiết sắc nét"
        ),
        "tag_color": (255, 100, 100),
    },
    {
        "id": "tiny_subtle",
        "title": "TEST 3: ĐIỀU KHIỂN KÍCH THƯỚC SIÊU NHỎ",
        "subtitle": "Tiny Subtle Size / Minimalist Stone Base",
        "text": "PHIÊN BẢN GIỚI HẠN",
        "font": "bevietnam",
        "prompt": (
            "Không gian triển lãm nghệ thuật tối giản cao cấp, một khối bệ đá cẩm thạch đen tuyền ở trung tâm, "
            "dòng chữ tiêu đề kích thước siêu nhỏ tinh tế thanh mảnh màu bạc bạch kim (tiny subtle platinum silver text) "
            "khắc chìm khiêm tốn ở một góc nhỏ dưới chân bệ đá, ánh sáng dịu tương phản cao, đổ bóng chân thực, chi tiết sắc nét"
        ),
        "tag_color": (160, 210, 255),
    },
    {
        "id": "emerald_neon",
        "title": "TEST 4: MÀU SẮC ĐẶC THÙ (XANH NGỌC LỤC BẢO)",
        "subtitle": "Vivid Emerald Green Neon / Hologram Glow",
        "text": "NĂNG LƯỢNG XANH",
        "font": "bevietnam",
        "prompt": (
            "Thành phố công nghệ tương lai hiện đại phong cách cyberpunk, "
            "dòng chữ tiêu đề đèn neon phát quang màu xanh ngọc lục bảo (vivid emerald green neon text) "
            "rực rỡ với hiệu ứng ánh sáng hologram ba chiều tỏa quầng sáng lung linh huyền ảo, "
            "ánh sáng ban đêm tương phản cao, đổ bóng chân thực, chi tiết sắc nét"
        ),
        "tag_color": (80, 255, 180),
    },
]
...
def stitch_2x2_comparison_panel(
    ...
    title_text = "KIỂM CHỨNG NĂNG LỰC ĐIỀU KHIỂN HÌNH HỌC KHÔNG GIAN & MÀU SẮC BẰNG PROMPT"
    bbox_t = font_title.getbbox(title_text)
    draw.text(((total_w - (bbox_t[2] - bbox_t[0])) // 2, 18), title_text, fill=(255, 215, 80), font=font_title)

    sub_text = "Thử nghiệm: Chữ uốn cong hình cầu (Trái Đất/Bóng đá) | Kích thước siêu nhỏ | Màu sắc đặc thù (Emerald Green)"
    bbox_s = font_sub.getbbox(sub_text)
    draw.text(((total_w - (bbox_s[2] - bbox_s[0])) // 2, 58), sub_text, fill=(195, 205, 225), font=font_sub)

    sub_text_2 = "Cấu hình: FLUX.2-klein-base-4B | T2I 9:16 (576x1024) | In-Context t = 10.0 | Seed: 42 (Cố định z_init)"
    ...
    footer_str = "Bản quyền R&D Tendoo AI | Nghiên cứu Biến dạng Hình học & Điều khiển Không gian 3D | 2026"
    ...
    print(f"\n📊 [Master 2x2 Grid Saved] -> {output_path} ({total_w}x{total_h})")
...
def run_benchmark(
    ...
    guidance: float = 4.5,
    seed: int = 42,
    output_dir: str = "output_prompt_spatial_benchmark",
    ...
    print("=" * 80)
    print("🔬 TENDOO AI - PROMPT SPATIAL & STYLISTIC CONTROL BENCHMARK (9:16)")
    print(f"📐 Canvas Resolution : 9:16 ({width}x{height})")
    print(f"⚙️  Denoise Config    : {num_steps} steps | CFG {guidance} | Seed: {seed}")
    print(f"🎯 Test Scenarios    : {len(SCENARIOS)} cases (Curvature, Size, Color)")
    print("=" * 80)
    ...
    generated_results = []
    print("\n[2/3] Executing 4 Benchmark Runs...")

    for idx, sc in enumerate(SCENARIOS):
        print("-" * 75)
        print(f"⚡ [{idx + 1}/{len(SCENARIOS)}] Running: {sc['title']}...")
    ...
    grid_file = out_path / "PROMPT_SPATIAL_CONTROL_PANEL.png"
    ...
    print("\n" + "=" * 80)
    print("🎉 PROMPT SPATIAL & STYLISTIC CONTROL BENCHMARK COMPLETED!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Tendoo AI - Prompt Spatial & Stylistic Control Benchmark"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_prompt_spatial_benchmark",
