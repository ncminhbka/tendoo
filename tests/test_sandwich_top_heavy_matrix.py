...
def get_backdrop_uri(ratio_name: str, w: int = 1024, h: int = 1024) -> str:
    """Tạo ảnh nền mock studio trung tính (Clean Dark Studio Vignette SVG) theo yêu cầu người dùng."""
    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}">
  <defs>
    <radialGradient id="mockStudioBg" cx="50%" cy="44%" r="68%">
      <stop offset="0%" stop-color="#1E293B"/>
      <stop offset="55%" stop-color="#0F172A"/>
      <stop offset="100%" stop-color="#070B14"/>
    </radialGradient>
  </defs>
  <rect width="100%" height="100%" fill="url(#mockStudioBg)"/>
</svg>"""
    import base64
...
def test_sandwich_html_no_scrim_and_features():
    ...
    bg_uri = get_backdrop_uri("1_1")
    html = build_template_html(plan, bg_uri, w, h)
...
def test_sandwich_12_matrix_rendering():
    ...
    for ratio_name, (w, h) in RATIOS.items():
        bg_uri = get_backdrop_uri(ratio_name, w, h)
