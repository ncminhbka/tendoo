...
1. Fallback Heuristic & Prompt Authority (Prompt > Form, Suppression rules, Orientation).
2. Chameleon Adaptive Palette Calculation on synthetic image regions.
3. Jinja2 compilation & HTML generation across all templates with autofit markers.
4. Playwright autofit contract verification (__tendooAutofitDone flag check).
"""
...
def test_fallback_heuristics_and_suppression():
    print("--> [1/4] Testing Fallback Heuristics & Prompt Authority...")
    ...
    # 1.5 Test retro/vintage prompt still routes to a warm background tone + serif font
    p5 = fallback_heuristic_planner({"product_name": "Thời Trang"}, prompt="phong cách bìa tạp chí retro film grain")
    assert p5.style.background_tone == "warm_rustic", f"Expected background_tone=warm_rustic, got {p5.style.background_tone}"
...
def test_chameleon_adaptive_palette():
    print("--> [2/4] Testing Chameleon Adaptive Palette...")
...
def test_autofit_contract():
    print("--> [3/4] Testing Autofit Contract & JS Flag...")
...
def test_all_11_templates_html_compilation():
    print("--> [4/4] Compiling & Rendering HTML across all templates...")
    ...
    print("\n=======================================================")
    print("🎉 ALL 4 TEST SUITES PASSED FLAWLESSLY! TENDOO V3 READY.")
    print("=======================================================\n")
