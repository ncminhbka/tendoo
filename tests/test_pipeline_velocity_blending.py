...
def test_mask_gamma_default_matches_original_unboosted_behavior():
    """mask_gamma=1.0/corridor_guidance_boost=0.0 (the defaults) must be a complete
    no-op -- bit-exact identical to not passing either knob at all, since production
    callers (demo_server.py) never pass them and the bit-exact regression tests above
    must keep passing unchanged (2026-09-14: these 2 knobs were added purely for
    scripts/probe_diffusion_mask_adherence.py's real-GPU experimentation, see
    velocity_blending.py's updated docstring)."""
    num_canvas, num_steps = 12, 8
    inputs = _make_inputs(seed=99, num_canvas=num_canvas, num_ref=0, ctx_len=6)
    img, img_ids, txt_scene, txt_scene_ids, txt_corridor, txt_corridor_ids, spatial_mask = inputs
    timesteps = list(torch.linspace(1.0, 0.0, num_steps).tolist())
    model = _FakeFlux2Model()

    out_default = pipeline_e2e_poster.denoise_regional_velocity_blended(
        model, img.clone(), img_ids.clone(), txt_scene.clone(), txt_scene_ids.clone(),
        txt_corridor.clone(), txt_corridor_ids.clone(), spatial_mask.clone(),
        timesteps, guidance=1.5, num_canvas_tokens=num_canvas,
    )
    out_explicit = pipeline_e2e_poster.denoise_regional_velocity_blended(
        model, img.clone(), img_ids.clone(), txt_scene.clone(), txt_scene_ids.clone(),
        txt_corridor.clone(), txt_corridor_ids.clone(), spatial_mask.clone(),
        timesteps, guidance=1.5, num_canvas_tokens=num_canvas,
        mask_gamma=1.0, corridor_guidance_boost=0.0,
    )
    assert torch.equal(out_default, out_explicit)


def test_mask_gamma_below_one_increases_corridor_influence_at_partial_mask_values():
    """M' = M**gamma with gamma<1 pushes any partial (0<M<1) mask value TOWARD 1.0 --
    i.e. strengthens corridor influence in the mask's soft transition zone, without
    changing pixels that are already exactly 0 or 1 (0**g == 0, 1**g == 1 for any g)."""
    num_canvas = 8
    g = torch.Generator().manual_seed(5)
    spatial_mask = torch.tensor(
        [[[0.0], [0.4], [1.0], [0.4], [0.0], [1.0], [0.4], [0.0]]], dtype=torch.float32,
    )
    img = torch.randn(1, num_canvas, 8, generator=g)
    img_ids = torch.randn(1, num_canvas, 4, generator=g)
    txt_scene = torch.randn(1, 6, 8, generator=g)
    txt_scene_ids = torch.randn(1, 6, 4, generator=g)
    txt_corridor = torch.randn(1, 6, 8, generator=g)
    txt_corridor_ids = torch.randn(1, 6, 4, generator=g)
    timesteps = list(torch.linspace(1.0, 0.0, 4).tolist())
    model = _FakeFlux2Model()

    out_gamma1 = pipeline_e2e_poster.denoise_regional_velocity_blended(
        model, img.clone(), img_ids.clone(), txt_scene.clone(), txt_scene_ids.clone(),
        txt_corridor.clone(), txt_corridor_ids.clone(), spatial_mask.clone(),
        timesteps, guidance=1.5, num_canvas_tokens=num_canvas, mask_gamma=1.0,
    )
    out_gamma_half = pipeline_e2e_poster.denoise_regional_velocity_blended(
        model, img.clone(), img_ids.clone(), txt_scene.clone(), txt_scene_ids.clone(),
        txt_corridor.clone(), txt_corridor_ids.clone(), spatial_mask.clone(),
        timesteps, guidance=1.5, num_canvas_tokens=num_canvas, mask_gamma=0.5,
    )
    assert not torch.allclose(out_gamma1, out_gamma_half), (
        "mask_gamma=0.5 should change the blended output at partial-mask positions"
    )


def test_corridor_guidance_boost_only_affects_corridor_branch():
    """The fake model's velocity depends on `guidance` (see _FakeFlux2Model's
    g_signature) -- boosting ONLY the corridor branch's guidance must change the
    final output wherever mask>0 (the corridor branch actually contributes there),
    while a boost of exactly 0.0 must remain a complete no-op."""
    num_canvas, num_steps = 10, 5
    inputs = _make_inputs(seed=11, num_canvas=num_canvas, num_ref=0, ctx_len=6)
    img, img_ids, txt_scene, txt_scene_ids, txt_corridor, txt_corridor_ids, spatial_mask = inputs
    # Ensure mask values are all > 0 so the corridor branch actually contributes to
    # v_blend everywhere (a boost applied where mask==0 would be invisible in the
    # output, which would make this test meaningless).
    spatial_mask = spatial_mask.clamp(min=0.3)
    timesteps = list(torch.linspace(1.0, 0.0, num_steps).tolist())
    model = _FakeFlux2Model()

    out_no_boost = pipeline_e2e_poster.denoise_regional_velocity_blended(
        model, img.clone(), img_ids.clone(), txt_scene.clone(), txt_scene_ids.clone(),
        txt_corridor.clone(), txt_corridor_ids.clone(), spatial_mask.clone(),
        timesteps, guidance=1.5, num_canvas_tokens=num_canvas, corridor_guidance_boost=0.0,
    )
    out_boosted = pipeline_e2e_poster.denoise_regional_velocity_blended(
        model, img.clone(), img_ids.clone(), txt_scene.clone(), txt_scene_ids.clone(),
        txt_corridor.clone(), txt_corridor_ids.clone(), spatial_mask.clone(),
        timesteps, guidance=1.5, num_canvas_tokens=num_canvas, corridor_guidance_boost=2.0,
    )
    assert not torch.allclose(out_no_boost, out_boosted), (
        "a nonzero corridor_guidance_boost should change the blended output"
    )
