...
def test_evaluate_case_output_pass():
    case = BenchmarkCase(
        ...
        expected_headline_keywords=["GRAND OPENING"],
        expected_zones=["middle_center"],
        forbidden_scene_strings=["GRAND OPENING", "16:9"],
    )
    raw_json = json.dumps({
        "extra_blocks": [
            {"field": None, "text": "GRAND OPENING", "zone": "middle_center", "size": "xlarge", "container": "none"},
            {"field": None, "text": "Menu mới", "zone": "middle_center", "size": "small", "container": "pill"},
        ],
