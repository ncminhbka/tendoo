...
from tendoo.core.style import DEFAULT_LAYOUT, resolve_style_preset
from tendoo.engine.blocks import AdaptiveBlock, map_category_to_default_blocks
...
def test_zone_names_consistency_no_center():
    ...
    # ZONE_NAMES = ["top_bar", "bottom_bar", "middle_center"] + GRID_ZONE_NAMES(8) = 11
    # (matches VALID_ZONES in engine/blocks.py) -- was stale at 10 (off by one, predates
    # one of top_bar/bottom_bar/middle_center being added to the current zone set).
    assert len(ZONE_NAMES) == 11, "Total valid zones must be 11"
...
def test_store_info_preserved_with_free_text_blocks():
    ...
    custom_plan_blocks = [
        {"text": "FLASH SALE 50%", "zone": "top_center", "size": "xlarge"},
        {"text": "MUA 2 TẶNG 1", "zone": "middle_right", "size": "medium", "container": "pill"},
    ]
...
def test_store_info_top_bar_override():
    ...
    layout = OmniBlockLayout()
    top_plan_blocks = [
        {"text": "TIÊU ĐỀ NỔI BẬT", "zone": "top_center", "size": "xlarge"},
        {
...
def test_category_visual_variants_rendering():
    ...
    # 1. Promo: pill_badge on the `applied_product` field -- NOT `discount`, which
    # engine/blocks.py's promo branch renders as plain pure-text with no style_variant
    # at all (only `applied_product` gets style_variant="pill_badge"); this assertion
    # used to (incorrectly) check `discount` and would never pass against current source.
    promo_blocks = map_category_to_default_blocks(
        "promo", title="SALE", fields={"discount": "GIẢM 50%", "applied_product": "Toàn bộ sản phẩm"}
    )
    ap_block = next(b for b in promo_blocks if b.field == "applied_product")
    assert ap_block.style_variant == "pill_badge"
    ...
    # 4. Render HTML and verify CSS classes -- `offer_sub` feeds `applied_product` in
    # OmniBlockLayout._extract_blocks()'s form-fields fallback mapping (see engine/layout.py).
    content = PosterContent(headline="SALE", offer_main="GIẢM 50%", offer_sub="Toàn bộ sản phẩm", category="promo")
