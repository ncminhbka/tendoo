...
def _flatten_wrapped_text(html: str) -> str:
    """A short pure-typography block (container='none', size in xlarge/large) whose text
    is >= 3 words and exceeds compute_block_metrics()'s per-zone one-line character
    budget gets balanced onto 2 lines by balance_vietnamese_headline() and joined with
    '<br>' in the rendered HTML (see engine/geometry.py) -- this is deliberate,
    working wrap behavior, not a bug. Assertions that check for an exact literal phrase
    (e.g. a product name or job title) must normalize '<br>' back to a space first, or
    they fail whenever that phrase happens to get word-wrapped."""
...
def test_generate_product_intro_adaptive_components(client):
    ...
    run_folder = demo_server.OUTPUT_DIR / Path(data["final_poster_url"]).parent.name
    html = (run_folder / "03_poster.html").read_text(encoding="utf-8")
    # "Sonic Pro Wireless" (3 words, 'large' pure-text) can legitimately get balanced
    # onto 2 lines ("Sonic Pro<br>Wireless") by the same wrap logic that applies to any
    # short pure-typography block -- normalize before checking (see _flatten_wrapped_text).
    assert "Sonic Pro Wireless" in _flatten_wrapped_text(html)
...
def test_generate_customer_feedback_adaptive_components(client):
    ...
    run_folder = demo_server.OUTPUT_DIR / Path(data["final_poster_url"]).parent.name
    html = (run_folder / "03_poster.html").read_text(encoding="utf-8")
    # (2026-09-13: .block-card (quote) now genuinely wraps long text across lines -- Fix C.)
    assert "Không gian cực kỳ thư thái" in _flatten_wrapped_text(html)
...
def test_generate_recruitment_adaptive_components(client):
    ...
    run_folder = demo_server.OUTPUT_DIR / Path(data["final_poster_url"]).parent.name
    html = (run_folder / "03_poster.html").read_text(encoding="utf-8")
    # See _flatten_wrapped_text: "SENIOR AI RESEARCH ENGINEER" can legitimately balance
    # onto 2 lines ("SENIOR AI<br>RESEARCH ENGINEER").
    assert "SENIOR AI RESEARCH ENGINEER" in _flatten_wrapped_text(html)
    # (2026-09-13: card/pill blocks now also wrap across multiple lines (Fix C -- see
    # geometry.py::compute_block_metrics), same as the pure-typography case above.)
    assert "Thu nhập up to $4,000" in _flatten_wrapped_text(html)
...
def test_generate_usage_guide_adaptive_components(client):
    ...
    run_folder = demo_server.OUTPUT_DIR / Path(data["final_poster_url"]).parent.name
    html = (run_folder / "03_poster.html").read_text(encoding="utf-8")
    assert "Quét mã QR" in _flatten_wrapped_text(html)
    assert "Nhận hàng hỏa tốc" in _flatten_wrapped_text(html)
...
def test_generate_omni_adaptive_mock(client):
    ...
    # See _flatten_wrapped_text: "Tendoo Speed Elite" can legitimately balance onto 2 lines.
    assert "Tendoo Speed Elite" in _flatten_wrapped_text(html)
...
def test_all_categories_no_raw_emojis_and_valid_svg_icons(client):
    ...
    """
    from tendoo.core.typography import EMOJI_PATTERN
...
def _canned_plan(extra_blocks=None, scene_prompt="a clean sunlit studio background, unbranded"):
    """Builds a v2-schema render plan dict for monkeypatching try_fetch_render_plan.

    (2026-09-13: no more dedicated "title" key -- a prompt-requested headline is just
    another `extra_blocks` entry, field=null/size=xlarge/container=none, indistinguishable
    from any other ad-hoc content. See llm_render_plan_server.py's module docstring.)
    """
    return {
...
def test_generate_blank_field_extraction_stays_on_fixed_layout(client, monkeypatch):
    ...
    # (2026-09-13: feedback_quote renders in .block-card, which now wraps long text -- Fix C.)
    assert "Khách hàng cực kỳ hài lòng!" in _flatten_wrapped_text(html)
...
def test_generate_title_override_from_prompt_wins_over_form(client, monkeypatch):
    """2026-09-13: no more dedicated title decision -- a prompt-requested headline is
    just another ad-hoc extra_blocks entry (field=null, size=xlarge, container=none),
    which suppresses the mandatory form title the same way any other ad-hoc content
    does (see run_pipeline_inference's title-resolution comment) while still rendering
    itself. Kept deliberately SHORT (single line) -- balance_vietnamese_headline()
    ...
    plan = _canned_plan(extra_blocks=[
        {"field": None, "text": "PROMPTWINS", "zone": None, "size": "xlarge", "container": "none"},
    ])
    monkeypatch.setattr(demo_server, "try_fetch_render_plan", lambda req, fields: plan)
...
def test_zone_size_preference_never_auto_assigns_center():
    ...
    only an explicit zone='center'/'middle_center' on the block itself should ever land
    text there. (2026-09-12: `_ZONE_ROLE_PREFERENCE` was renamed `_ZONE_BLOCK_PREFERENCE`
    and keyed by `size`/`container` -- see AdaptiveBlock, there's no 'role' field.)"""
    from tendoo.demo_server import _ZONE_BLOCK_PREFERENCE, _auto_assign_zones

    for key, candidates in _ZONE_BLOCK_PREFERENCE.items():
        assert "middle_center" not in candidates and "center" not in candidates, (
            f"size/container '{key}' still prefers the center sanctuary zone"
        )

    # End-to-end: an unzoned block must not land on center as long as another named
    # zone remains free.
    blocks = [{"size": "xlarge", "text": "X", "zone": None}]
    ...
    explicit_blocks = [{"size": "xlarge", "text": "X", "zone": "center"}]
