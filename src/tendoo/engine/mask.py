...
def generate_omni_corridor_mask(
    ...
    # `occupied_zones`/has_top_bar/has_bottom_bar cho compute_block_metrics(): cùng khái
    # niệm "zone nào đang có nội dung" mà engine/layout.py::render_html dùng để thu hẹp
    # zone_max_w/zone_max_h theo zone lân cận -- truyền vào đây để đường ước lượng offline
    # PIL này khớp với ngân sách không gian thật, thay vì tính độc lập và có thể lệch nhau
    # (xem estimate_zone_available_box() trong geometry.py).
    occupied_zones = {(b.zone or "top_left") for b in blocks if (b.zone or "top_left") not in ("top_bar", "bottom_bar")}
    has_top_bar = any((b.zone or "") == "top_bar" for b in blocks)
    has_bottom_bar = any((b.zone or "") == "bottom_bar" for b in blocks)

    # Vẽ các vùng chữ vào mask
    for b in blocks:
        text = b.text.strip()
        # Khối QR (container="qr" hoặc có data_uri) không nhất thiết có caption text --
        # KHÔNG được bỏ qua chỉ vì text rỗng, nếu không mã QR sẽ không được bảo vệ chút
        # nào trong mask (compute_block_metrics đã có nhánh riêng xử lý QR không cần text).
        is_qr_block = b.container == "qr" or bool(getattr(b, "data_uri", None))
        if not text and not is_qr_block:
            continue

        metrics = compute_block_metrics(
            b, font_path, width, height,
