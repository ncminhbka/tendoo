...
# Trần thời gian tuyệt đối cho 1 lần render/measure qua Chromium (2026-09-12 fix):
# trước đây `future.result()` không có timeout -- nếu tiến trình Chromium bị treo (hoặc
# hệ thống đã tích tụ nhiều tiến trình Chromium mồ côi do thiếu `finally` ở nơi khác),
# request gọi vào đây sẽ treo vô thời hạn, không có cách nào phục hồi. Tất cả các
# `wait_for`/timeout nội bộ trong `_set_content_and_wait_ready` chỉ giới hạn từng bước
# nạp trang (tối đa ~6s+2s+4s+3s cộng dồn) -- hằng số này là lưới an toàn ở tầng ngoài
# cùng, bao trọn cả việc khởi chạy trình duyệt.
...
def _run_coroutine_sync(coro: Coroutine[Any, Any, T]) -> T:
    ...
    if loop and loop.is_running():
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            return future.result(timeout=_RENDER_TIMEOUT_S)
    else:
...
async def _set_content_and_wait_ready(page: Any, html_content: str) -> None:
    ...
    try:
        ...
        try:
            ...
            # Before 2026-09-12 this was a bare `except Exception: pass` -- completely
            # silent. Execution would still fall through to the font-ready wait, the
            # autofit wait, and finally page.screenshot(), which succeeds and silently
            # produces a blank/broken PNG for a real poster request with zero trace in
            # the logs of what went wrong. Raise instead: demo_server.py's api_generate
            # already wraps the whole pipeline call in a try/except that turns any
            # exception into a clean HTTP 500 -- an honest error beats a silent blank image.
            logger.error(f"[PosterRenderer] Both content-load attempts failed (networkidle: {e}; domcontentloaded: {e2}); aborting rather than screenshotting a blank page.")
            raise RuntimeError(f"Failed to load HTML into headless Chromium after 2 attempts: {e2}") from e2
    ...
    async def render_html_async(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # được đóng tường minh (chỉ trông chờ vào teardown ngầm của
                # `async_playwright()`), rủi ro tích tụ tiến trình Chromium mồ côi trên
                # server chạy dài hạn dưới các lần lỗi lặp lại.
                await browser.close()
    ...
    async def measure_zone_rects_async(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # See render_html_async's matching fix (2026-09-12): guarantee cleanup
                # even if context/page setup or evaluate() raises unexpectedly.
                await browser.close()
