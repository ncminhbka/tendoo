...
29. **CHỐT (LOCKED) GLYPH ENGINE — ĐỊNH LUẬT DẢI TỈ LỆ KHUNG HÌNH TỰ THÂN CỦA BOX & SÀN CỠ CHỮ HỢP NHẤT 40PT (THE GLYPH-BOX SELF-ASPECT-RATIO BAND & UNIFIED 40PT FLOOR LAW)** — thay thế hoàn toàn bảng floor theo nhóm font ở Rule 26 phía trên (`src/tendoo/glyph_engine.py`, đã kiểm chứng qua 7 vòng probe GPU liên tiếp, `scripts/probe_glyph_*.py`):
    - **Hành trình bác bỏ 2 giả thuyết trực giác trước khi tìm ra biến thật**:
      1. *"Model bảo toàn số dòng, glyph rộng sẽ vỡ khi ép vào canvas hẹp"* — **BỊ BÁC BỎ**: cùng một bitmap glyph y hệt (bài thơ 4 dòng, 608×512px) render hoàn hảo trên canvas 1024×576 nhưng fail hoàn toàn trên canvas 576×1024 — số dòng, font size, nội dung giữ nguyên, chỉ đổi hướng canvas.
      2. *"Tỉ lệ glyph_latent_width / canvas_latent_width phải ≤ ~0.6"* — **BỊ BÁC BỎ** bởi chính 1 kết quả sản xuất đã xác nhận 100% từ trước (case Tây Tiến, exp54, `demo_tendoo_poster.py --box_w 896 --box_h 512` trên canvas 1024×1024, ratio=0.875 — lẽ ra phải fail theo giả thuyết này) và bởi 1 vòng probe cô lập cho thấy ngay cả ratio ≤0.4 vẫn fail nếu glyph quá dẹt — chứng minh ratio-với-canvas chỉ là biến tương quan ngẫu nhiên ở phần lớn case trước đó, không phải biến nhân quả.
      3. **BIẾN THẬT (xác nhận độc lập 2 lần)**: **tỉ lệ khung hình của CHÍNH glyph box** (width_px/height_px), hoàn toàn độc lập với canvas. Cả 2 thí nghiệm (sweep target_lines=1→4 với canvas-ratio giữ cố định ≤0.35; và ma trận 2×2 canvas×chiến-lược-box) đều cho **cùng 1 đường cong đơn điệu** trên đúng biến này: aspect tự thân trong khoảng **[0.5, 1.3]** thì render ổn định; càng lệch xa 1.0 theo cả 2 hướng (quá dẹt HOẶC quá cao-hẹp) càng dễ hỏng, tới 0% ở cực đoan (aspect 5.4). Điều này giải thích trọn vẹn case Tây Tiến: aspect tự thân của nó = 896/512 = 1.75 — nằm trong dải an toàn, dù ratio-với-canvas (0.875) trông có vẻ "nguy hiểm" theo giả thuyết (2).
    - **Luật đã triển khai trong code**: `compute_optimal_glyph_box` (khi không có override tường minh: `force_single_line`/`target_lines`/`\n` thủ công) tìm số dòng N **nhỏ nhất** sao cho `width_px/height_px` rơi vào [0.5, 1.3]. `target_canvas_w/h` giờ chỉ là chặn an toàn lỏng (glyph không được rộng hơn cả canvas), không còn là đòn bẩy chính.
    - **Sàn cỡ chữ hợp nhất 40pt (thay thế bảng theo nhóm font A/B/C ở Rule 26)**: sau khi (3) giải quyết xong lỗi cấu trúc lớn (vỡ layout, chữ đè nhau), lộ ra 1 lỗi nhỏ đồng đều hơn: dấu phụ (dấu mũ, dấu ngã, gạch ngang chữ "đ") sai nhẹ ở floor cũ (32pt bevietnam), trong khi chữ Latin cơ bản luôn đúng — độc lập với aspect ratio. Sweep multi-seed cô lập đúng biến font-size (giữ aspect cố định trong dải an toàn) trên `bevietnam` cho kết quả: 24-30pt vẫn lỗi dấu, 36pt+ sạch, **40pt sắc nét nhất**. Khớp với việc dò ngược đúng thuật toán binary-search của chính công thức Tây Tiến: nó chọn **48pt** cho playfair ở 4 dòng — xác nhận các floor cũ trên toàn dự án (32/36/40-48pt theo nhóm) đều bị khóa thấp hơn mức cần cho độ tinh xảo dấu phụ. **Toàn bộ 16 font đã được nâng đồng loạt lên 40pt** — chỉ riêng bevietnam được đo trực tiếp, 15 font còn lại nâng theo quyết định thống nhất bảo thủ (40 > mọi floor cũ), chưa được đo riêng từng font.
    - **Rủi ro còn tồn đọng, đã biết trước**: kể cả trong dải an toàn + floor mới, vẫn có tỷ lệ fail ngẫu nhiên theo seed (1 case "short" 1 dòng trước đó chỉ sạch đúng 1/3 seed dù cấu hình coi như an toàn) — quy trình sản xuất cần tính đến regenerate/retry, không kỳ vọng 100% một lần ăn ngay. Padding an toàn (16px) và floor chiều cao tối thiểu mỗi dòng vẫn chưa "khóa cứng" vì cùng lý do nhiễu seed.
    - **TRẠNG THÁI**: tầng render glyph đơn lẻ (cô lập tại t=10) coi như **ĐÃ CHỐT**. Giai đoạn tiếp theo của dự án chuyển sang bài toán đa khối (RoPE spatial binding, Regional Parallel Diffusion) xây dựng trên nền glyph đơn lẻ đã đáng tin cậy này.
