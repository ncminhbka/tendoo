...
33. **ĐỊNH LUẬT CẤM ĐƯA CHUỖI CHỮ THẬT VÀO PROMPT — TÍN HIỆU NGỮ NGHĨA CẠNH TRANH PHÁ HỎNG NGAY CẢ ĐƠN KHỐI (THE NO-LITERAL-TEXT-IN-PROMPT LAW)** (`scripts/probe_sequential_inpainting.py --quote_text`, đối chứng A/B seed-cho-seed với baseline):
    - **Giả thuyết đã kiểm chứng**: crosstalk và việc không kiểm soát được kích thước (subtitle to bằng title ở Direction 4) có thể do THIẾU tín hiệu ràng buộc (binding) giữa "prompt nói chỉ dẫn vai trò/vị trí/kích thước nào" và "reference slot nào (t=10 hay t=20) mang nội dung đó" — vì chuỗi chữ thật đã bị xoá khỏi prompt (đúng thiết kế gốc, để tránh Qwen3 tự vẽ chữ sai). Thử đưa chuỗi chữ thật trở lại prompt (trong ngoặc kép, gắn liền với từ chỉ vai trò/vị trí, ví dụ `dòng chữ tiêu đề "TUYỂN DỤNG NHÂN TÀI" ... ở phía trên`), giữ nguyên 100% mọi thứ khác (glyph reference, mask, t offset, seed) để so sánh trực tiếp.
    - **Kết quả — BỊ BÁC BỎ DỨT KHOÁT, và tệ hơn cả không làm gì**: không chỉ subtitle không cải thiện, mà **Title — vốn bulletproof 100% xuyên suốt toàn bộ quá trình điều tra (Rule 26, 29, 31), chạy ở Pass 1 hoàn toàn cô lập, không hề có crosstalk hay mask nào** — cũng render sai ngay khi thêm đúng 1 biến này.
    - **Bản chất khoa học**: Qwen3 xử lý chuỗi trong ngoặc kép đúng như một tác vụ text-to-image thông thường — tức là một **nội dung cần được vẽ**, không phải một nhãn định danh trung tính. Nhưng Qwen3 **không hiểu hình dạng nét chữ tiếng Việt đúng** (đây chính xác là lý do dự án phải tiêm glyph VAE reference ngay từ đầu). Kết quả: DiT nhận **2 tín hiệu "cần vẽ chữ ở đây" cạnh tranh nhau** tại cùng một vị trí — một từ glyph VAE (chính xác tuyệt đối, mù ngữ nghĩa) và một từ embedding ngữ nghĩa của Qwen3 (đúng ngữ nghĩa, mù hình dạng, dễ kéo theo glyph sai/mờ) — và tín hiệu thứ hai đủ mạnh để nhiễu loạn output dù tín hiệu thứ nhất đúng.
    - **Ý nghĩa quyết định**: xác nhận lại quyết định thiết kế gốc của dự án (xoá chuỗi chữ khỏi prompt trước khi encode) là **bắt buộc, không phải sơ suất có thể nới lỏng**. Giả thuyết "thiếu tín hiệu binding" giữa prompt và glyph reference vẫn có thể đúng về mặt CHẨN ĐOÁN nguyên nhân crosstalk/khó kiểm soát kích thước, nhưng "đưa chuỗi chữ vào prompt" là **một cách SAI để chữa nó** — cần một cơ chế binding không đòi hỏi Qwen3 "đọc hiểu" nội dung chữ theo nghĩa đen (ví dụ: gán vai trò/kích thước qua cấu trúc không gian cứng như Direction 3's mask, không qua ngôn ngữ).
    - **QUYẾT ĐỊNH**: đóng hẳn hướng "quote text in prompt" dưới mọi biến thể (kể cả chỉ quote 1 khối, hay dùng cách diễn đạt nhẹ hơn) — cơ chế gốc rễ (Qwen3 mù hình dạng, luôn cạnh tranh với glyph VAE) sẽ tái diễn bất kể cách diễn đạt câu chữ.
